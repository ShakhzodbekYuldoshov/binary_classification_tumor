...
def load(set_='train', label=True):
    x = []
    y = []
    path = BASEDIR+'/dataset/'+set_

    # load dangerous dataset
    for name in tqdm.tqdm(
            os.listdir(BASEDIR+'/dataset/'+set_+'/dangerous'),
            desc='{:{}}'.format('Load dangerous dataset', SPACE),
            unit_scale=UNIT_SCALE):
        img = Image.open(path+'/dangerous/'+name)
    ...
    # load normal dataset
    for name in tqdm.tqdm(
            os.listdir(BASEDIR+'/dataset/'+set_+'/normal'),
            desc='{:{}}'.format('Load normal dataset', SPACE),
            unit_scale=UNIT_SCALE):
        img = Image.open(path+'/normal/'+name)
        img = np.asarray(img, np.float64)
        class_ = 1
        if not label:
            class_ = class_[:class_.rfind('.')] + '.png'
